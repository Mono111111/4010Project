# train_sarsa.py
import numpy as np
import matplotlib.pyplot as plt
from agents.q_learning import QLearningAgent
from agents.sarsa import SARSA_Agent, get_state
from agents.dqn import DQN_Agent
from environment.gym_env import SnakeGymEnv


# =========================
# Switch mode here: "train" or "test"
# =========================
MODE = "train" 

def train_and_plot():
    """Train Q-learning agent and plot reward curve."""
    print(" Starting Q-Learning training...")
    
    env = SnakeGymEnv(render_mode=None)
    agent = QLearningAgent(
        env,
        alpha=0.1,
        gamma=0.95,
        epsilon=1.0,
        epsilon_min=0.05,
        epsilon_decay=0.995,
        episodes=1000,
    )
    
    rewards = agent.train()
    agent.save("q_learning_agent.pkl")
    
    # Plot episode reward curve
    plt.figure(figsize=(10, 6))
    plt.plot(rewards, alpha=0.6, label="Episode Reward")
    
    # Plot moving average
    window = 50
    if len(rewards) >= window:
        moving_avg = np.convolve(rewards, np.ones(window)/window, mode='valid')
        plt.plot(range(window-1, len(rewards)), moving_avg, 
                 color='red', linewidth=2, label=f"Moving Avg ({window} episodes)")
    
    plt.xlabel("Episode")
    plt.ylabel("Total Reward")
    plt.title("Q-Learning Training Reward Curve")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("q_learning_rewards.png")
    print(" Reward curve saved to q_learning_rewards.png")
    plt.show()

def test_trained():
    """Load trained Q-table and visualize one episode (greedy policy)."""
    print(" Testing trained Q-Learning agent...")
    
    env = SnakeGymEnv(render_mode="human")
    agent = QLearningAgent(env)
    agent.load("q_learning_agent.pkl")
    
    # No exploration during testing
    agent.epsilon = 0.0
    
    obs, info = env.reset()
    done = False
    total_reward = 0.0
    steps = 0
    
    print(" Starting test episode...")
    
    state = agent.get_state(env)

    while not done:
        q_values = [agent.Q[(state, a)] for a in range(agent.n_actions)]
        action = int(np.argmax(q_values))
        
        obs, reward, terminated, truncated, info = env.step(action)
        done = terminated or truncated
        total_reward += reward
        steps += 1
        state = agent.get_state(env)
        
        env.render()
        env.game.clock.tick(10)  # Control speed, around 10 FPS
    
    print(f"\n Test finished!")
    print(f"   Total Reward: {total_reward:.2f}")
    print(f"   Final Score: {info['score']}")
    print(f"   Steps: {steps}")
    print(f"   Energy: {info['energy']}")

def train_sarsa():
    """Train SARSA agent."""
    print(" Starting SARSA training...")
    
    env = SnakeGymEnv(render_mode=None)
    agent = SARSA_Agent(
        env,
        alpha=0.1,
        gamma=0.95,
        epsilon=1.0,
        epsilon_min=0.05,
        epsilon_decay=0.995,
        episodes=1000,
    )
    
    rewards = agent.SARSA()
    agent.save("sarsa_agent.pkl")
    
    # Plot episode reward curve
    plt.figure(figsize=(10, 6))
    plt.plot(rewards, alpha=0.6, label="Episode Reward")
    
    # Plot moving average
    window = 50
    if len(rewards) >= window:
        moving_avg = np.convolve(rewards, np.ones(window)/window, mode='valid')
        plt.plot(range(window-1, len(rewards)), moving_avg, 
                 color='red', linewidth=2, label=f"Moving Avg ({window} episodes)")
    
    plt.xlabel("Episode")
    plt.ylabel("Total Reward")
    plt.title("SARSA Training Reward Curve")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("sarsa_rewards.png")
    print(" Reward curve saved to sarsa_rewards.png")
    plt.show()

    return rewards

def train_dqn():
    print(" Starting DQN training...")
    
    env = SnakeGymEnv(render_mode=None)
    agent = DQN_Agent(
        env,
        gamma=0.99,
        batch_size=64,
        lr=1e-3,
        replay_capacity=100000,
        epsilon_start=1.0,
        epsilon_end=0.05,
        epsilon_decay_steps=50_000,
        max_steps_per_episode=2000,
        hidden_dim=128,
    )
    
    history = agent.train(max_episode=500)
    rewards = history['episode_rewards']
    agent.save("dqn_snake.pt")
    
    # Plot episode reward curve
    plt.figure(figsize=(10, 6))
    plt.plot(rewards, alpha=0.6, label="Episode Reward")
    
    # Plot moving average
    window = 50
    if len(rewards) >= window:
        moving_avg = np.convolve(rewards, np.ones(window) / window, mode="valid")
        plt.plot(
            range(window - 1, len(rewards)),
            moving_avg,
            linewidth=2,
            label=f"Moving Avg ({window} episodes)",
        )
    
    plt.xlabel("Episode")
    plt.ylabel("Total Reward")
    plt.title("DQN Training Reward Curve")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("dqn_rewards.png")
    print(" Reward curve saved to dqn_rewards.png")
    plt.show()

def test_trained():
    """Load trained Q-table and visualize one episode (greedy policy)."""
    print(" Testing trained Q-Learning agent...")
    
    env = SnakeGymEnv(render_mode="human")
    agent = QLearningAgent(env)
    agent.load("q_learning_agent.pkl")
    
    # No exploration during testing
    agent.epsilon = 0.0
    
    obs, info = env.reset()
    done = False
    total_reward = 0.0
    steps = 0
    
    print(" Starting test episode...")
    
    state = agent.get_state(env)

    while not done:
        q_values = [agent.Q[(state, a)] for a in range(agent.n_actions)]
        action = int(np.argmax(q_values))
        
        obs, reward, terminated, truncated, info = env.step(action)
        done = terminated or truncated
        total_reward += reward
        steps += 1
        state = agent.get_state(env)
        
        env.render()
        env.game.clock.tick(10)  # Control speed, around 10 FPS
    
    print(f"\n Test finished!")
    print(f"   Total Reward: {total_reward:.2f}")
    print(f"   Final Score: {info['score']}")
    print(f"   Steps: {steps}")
    print(f"   Energy: {info['energy']}")

def test_sarsa():
    print(" Testing trained SARSA agent...")
    
    env = SnakeGymEnv(render_mode="human")
    agent = SARSA_Agent(env)
    agent.load("sarsa_agent.pkl")
    
    # No exploration during testing
    agent.epsilon = 0.0
    
    obs, info = env.reset()
    done = False
    total_reward = 0.0
    steps = 0
    
    print(" Starting test episode...")
    
    state = get_state(env)

    while not done:
        q_values = [agent.Q[(state, a)] for a in range(agent.n_actions)]
        action = int(np.argmax(q_values))
        
        obs, reward, terminated, truncated, info = env.step(action)
        done = terminated or truncated
        total_reward += reward
        steps += 1

        state = get_state(env)
        
        env.render()
        env.game.clock.tick(10)  # Control speed, around 10 FPS
    
    print(f"\n Test finished!")
    print(f"   Total Reward: {total_reward:.2f}")
    print(f"   Final Score: {info['score']}")
    print(f"   Steps: {steps}")
    print(f"   Energy: {info['energy']}")

def test_dqn():
    print(" Testing trained DQN agent...")
    
    env = SnakeGymEnv(render_mode="human")
    agent = DQN_Agent(env)
    agent.load("dqn_snake.pt")
    
    n_episode = 1
    
    print(" Starting test episode...")
    agent.evaluate(n_episodes=n_episode, render=True)
    
    env.close()

if __name__ == "__main__":
    if MODE == "train":
        train_dqn()
    elif MODE == "test":
        test_dqn()
    else:
        print(" MODE must be 'train' or 'test'")